    ...
    <main className="mx-auto max-w-2xl p-6">
      ...
      <div className="mt-6 rounded border p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button className="rounded border px-3 py-1" onClick={() => navigator.clipboard.writeText(shareUrl)}>
            Copy link
          </button>
        </div>
      ...
      <div className="mt-8 rounded border p-4">
        ...
        <form className="mt-4 space-y-3" onSubmit={submitRsvp}>
          <div>
            ...
            <input className="mt-1 w-full rounded border p-2" value={guestName} onChange={(e) => setGuestName(e.target.value)} required />
          </div>

          <div>
            <label className="block text-sm font-medium">Response</label>
            <select className="mt-1 w-full rounded border p-2" value={status} onChange={(e) => setStatus(e.target.value as any)}>
          ...
          <div>
            ...
            <textarea className="mt-1 w-full rounded border p-2" rows={3} value={note} onChange={(e) => setNote(e.target.value)} />
          </div>
